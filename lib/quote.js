...
async function quote(pool, input, lock = false) {
  ...
  const [[composition]] = await pool.execute(`SELECT a.id AS atmosphere_id, a.title AS atmosphere_title,
    ...
    JOIN accents n ON n.id = aa.accent_id WHERE a.id = ? AND n.id = ? AND a.active = TRUE AND n.active = TRUE` + (lock ? ' FOR SHARE' : ''), [atmosphereId, accentId]);
  if (!composition) throw new InputError('Эта нота недоступна для выбранной атмосферы.', 422);
  const [[bottle]] = await pool.execute('SELECT id, name, volume_ml, price FROM bottles WHERE id = ? AND active = TRUE' + (lock ? ' FOR SHARE' : ''), [bottleId]);
  ...
  if (!Number.isSafeInteger(totalMinor)) throw new InputError('Стоимость превышает допустимую.', 422);
  return {
